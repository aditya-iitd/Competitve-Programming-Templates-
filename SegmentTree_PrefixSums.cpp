...
/* Segment Tree on Prefix Sums */
template <typename T>
class Segtree {
public:
    /* [minv : Minimum Prefix Sum] , [maxv : Maximum Prefix Sum] */
    struct node_t {
        T minv , maxv , sumv;
        node_t() {
            minv = maxv = sumv = (T) 0;
    ...
    node_t merge(node_t left , node_t right) {
        node_t mergedNode;
        mergedNode.minv = min(left.minv , left.sumv + right.minv);
        mergedNode.maxv = max(left.maxv , left.sumv + right.maxv);
